...
p_load(tidyverse, leaflet, leaflet.extras, viridis, sf, data.table, spatstat.utils)
...
#1.- Define study area
#**************************
df<-readRDS("Q:/dfad/users/jostou/home/22_09_16_tania_find fishing/data/df.rds")
...
coastline <- read_sf("Q:/gis/Dynamisk/GEOdata/BasicLayers/CoastLines/Denmark/Denmark_Simple.shp")#in UTM
...
land <-lengths(st_intersects(df_sf_utm, coastline_utm_land,parallel = 5)) > 0#
...
3) Logbook data: If logbook data is available, with start and end date-times, these can be used to define a trip, and cut off all points in harbor (depending on precision of logbook data). This is done by selecting gps position times that are within the reported trip times. 
...
#2) With harbour list - either define harbours with clusters or you already have a list of harbours

 
#3) use logbook data as source for trip definition 
book <- read.csv("log_data.csv")

setDT(df2)
setDT(book)

#unique trip start and end, with times... (maybe cases for NA needs handling ?)
trips <- book[ ,. (id = unique(log_book_nr)),
               by = .(device_id, trip_start, trip_end)]

#use only vessels where we have gps data 
trips <- trips[trips$device_id %in% df2$device_id, ]

#some formatting for "foverlaps", no duplicate columns allowed, but start and end both set to postime
